  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor(x: number, y: number) {
        ...
        // Create a gradient from orange to deep orange
        const hue = Math.random() * 30 + 15 // 15-45 range for oranges
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particlesArray.length; i++) {
        ...
        for (let j = i; j < particlesArray.length; j++) {
          ...
          if (distance < 30) {
            ctx.beginPath()
            ctx.strokeStyle = `rgba(255, 165, 0, ${0.2 - distance / 150})` // Orange color
            ctx.lineWidth = 0.5
